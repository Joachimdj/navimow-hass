...
class NavimowOAuth2Implementation(LocalOAuth2Implementation):
    ...
    async def _async_refresh_token(self, token: dict[str, Any]) -> dict[str, Any]:
        ...
        Navimow OAuth tokens expire after 1-2 days. When expired, HA attempts
        to refresh using grant_type=refresh_token. If the server doesn't
        support this or the refresh_token is expired, an exception is raised.
        ...
        - Transient failure (network timeout, DNS) -> raise as-is
        """
        ...
        try:
            ...
            # Server explicitly rejected (401/403/invalid/expired) -> need re-auth
            if any(
                k in err_str
                for k in (
                    "401",
            ...
            # Other errors (network, etc.) - raise as-is, don't immediately trigger re-auth
