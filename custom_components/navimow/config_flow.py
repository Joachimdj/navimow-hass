...
class NavimowOAuth2FlowHandler(
    ...
    async def async_step_user(
        ...
        if not CLIENT_ID or not CLIENT_SECRET:
            ...
            return self.async_abort(
                ...
                description_placeholders={
                    "error": "CLIENT_ID or CLIENT_SECRET not configured"
                },
            )
        ...
        # Only one OAuth2 implementation, proceed directly to authorize
    ...
    async def async_oauth_create_entry(self, data: dict[str, Any]) -> FlowResult:
        ...
        # HA automatically handles token exchange, data["token"] contains token info
        # For reauth, HA automatically updates the entry
        if self.source == config_entries.SOURCE_REAUTH:
            ...
            self.hass.config_entries.async_update_entry(
                ...
                data={
                    **existing_entry.data,
                    **data,  # Contains new token
                },
            )
            await self.hass.config_entries.async_reload(existing_entry.entry_id)
            return self.async_abort(reason="reauth_successful")

        # Save config and token (HA automatically handles token exchange)
        return self.async_create_entry(
            title="Navimow",
            data={
                "auth_implementation": DOMAIN,
                **data,  # Contains token (automatically handled by HA)
